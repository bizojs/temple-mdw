...
{:else}
    ...
    <div class="flex my-5 justify-center flex-wrap gap-3 text-2xl text-gray-200">
        {#if $page.data?.user?.callsign}
            <a href="/leo" class="rounded bg-gray-500 hover:bg-gray-400 transition-all px-5 py-2 flex gap-2 items-center">
                ...
                <span>
                    Law Enforcement
                    {#if $page.data?.user?.callsign}
                        <span class="text-blue-300">({$page.data?.user?.callsign})</span>
                    {/if}
                </span>
            </a>
        {:else}
